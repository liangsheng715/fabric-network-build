...
configtxgen -profile ThreeOrgsChannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID $CHANNEL_NAME
  ...
  echo "#################################################################"
configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/organization1MSPanchors.tx -channelID $CHANNEL_NAME -asOrg organization1MSP

  echo "#################################################################"
  echo "#######    Generating anchor peer update for organization2MSP   ##########"
  echo "#################################################################"
configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/organization2MSPanchors.tx -channelID $CHANNEL_NAME -asOrg organization2MSP

  echo "#################################################################"
  echo "#######    Generating anchor peer update for organization3MSP   ##########"
  echo "#################################################################"
configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/organization3MSPanchors.tx -channelID $CHANNEL_NAME -asOrg organization3MSP
...
NODE1_ORG3_CA=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization3.example.com/peers/node1.organization3.example.com/tls/ca.crt
...
docker exec -e CORE_PEER_TLS_ROOTCERT_FILE=$NODE1_ORG3_CA -e CORE_PEER_ADDRESS=node1.organization3.example.com:7051 -e CORE_PEER_LOCALMSPID=organization3MSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization3.example.com/users/Admin@organization3.example.com/msp cli peer channel join -b $CHANNEL_NAME.block
docker exec -e CORE_PEER_TLS_ROOTCERT_FILE=$NODE1_ORG3_CA -e CORE_PEER_ADDRESS=node2.organization3.example.com:7051 -e CORE_PEER_LOCALMSPID=organization3MSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization3.example.com/users/Admin@organization3.example.com/msp cli peer channel join -b $CHANNEL_NAME.block
...
docker exec -e CORE_PEER_TLS_ROOTCERT_FILE=$NODE1_ORG3_CA -e CORE_PEER_ADDRESS=node2.organization3.example.com:7051 -e CORE_PEER_LOCALMSPID=organization3MSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization3.example.com/users/Admin@organization3.example.com/msp cli peer channel update -o orderer.example.com:7050 -c $CHANNEL_NAME -f ./channel-artifacts/organization3MSPanchors.tx --tls true --cafile $ORDERER_CA
...
echo "Install chaincode on node1.org3..."
docker exec -e CORE_PEER_TLS_ROOTCERT_FILE=$NODE1_ORG3_CA -e CORE_PEER_ADDRESS=node1.organization3.example.com:7051 -e CORE_PEER_LOCALMSPID=organization3MSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization3.example.com/users/Admin@organization3.example.com/msp cli peer chaincode install -n $CHAINCODE_NAME -v 1.0 -l golang -p "github.com/chaincode/example"

echo "Instantiating chaincode on node1.org1..."
docker exec cli peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n $CHAINCODE_NAME -l golang -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "AND ('organization1MSP.peer','organization2MSP.peer','organization3MSP.peer')" --tls true --cafile $ORDERER_CA

sleep 5s

echo "Querying chaincode on node1.org2..."
docker exec -e CORE_PEER_TLS_ROOTCERT_FILE=$NODE1_ORG2_CA -e CORE_PEER_ADDRESS=node1.organization2.example.com:7051 -e CORE_PEER_LOCALMSPID=organization2MSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization2.example.com/users/Admin@organization2.example.com/msp cli peer chaincode query -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["query","a"]}'

sleep 5s

echo "Querying chaincode on node1.org3..."
docker exec -e CORE_PEER_TLS_ROOTCERT_FILE=$NODE1_ORG3_CA -e CORE_PEER_ADDRESS=node1.organization3.example.com:7051 -e CORE_PEER_LOCALMSPID=organization3MSP -e CORE_PEER_MSPCONFIGPATH=/opt/gopath/src/github.com/hyperledger/fabric/peer/crypto/peerOrganizations/organization3.example.com/users/Admin@organization3.example.com/msp cli peer chaincode query -C $CHANNEL_NAME -n $CHAINCODE_NAME -c '{"Args":["query","b"]}'
